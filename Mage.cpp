...
void Mage::display(){
    ...
    std::cout << getName() << " is a Level " << getLevel()<< " "<< getRace()<< " MAGE.";// output level, name and race
    ...
    std::cout <<"\n" << "School of Magic: "<< getSchool(); // output school of magic
    std::cout <<"\n" <<"Weapon: "<< getCastingWeapon(); // output casting weapon
    std::cout <<"\n"; 
    //output summoning status
    if (hasIncarnateSummon())
    {
        std::cout<<"They can summon an Incarnate.";
    }else{
        std::cout<<"They cannot summon an Incarnate.";
    }
    std::cout << "\n" <<"\n";
...
void Mage::eatTaintedStew()
{
    if (getRace() == "UNDEAD")
    {
        setVitality(getVitality()+3); // if characters is race undead, gain 3 vitality
    }else{
        setVitality(1); // else vitality =1;
        if (getCastingWeapon()== "WAND")
        {   
            setVitality(getVitality()+2); // if undead and has wand as casting weapon, gain 2 vitality points
        }else if(getCastingWeapon() == "STAFF"){
            setVitality(getVitality()+3); // if undead and has staff as casting weapon, gain 3 vitality points
        }
        if(hasIncarnateSummon()){
            setVitality(getVitality()+1); // if undead and has incarnate summon, gain 1 vitality points
        }
    }
}