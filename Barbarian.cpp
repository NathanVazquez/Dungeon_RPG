...
{
    std::cout << getName() << " is a Level " << getLevel()<< " "<< getRace()<< " BARBARIAN."; // output the name, level, race and subclass of the character
    std::cout << "\n" <<"Vitality: " <<getVitality(); //output vitality
    std::cout << "\n" << "Armor: " << getArmor(); // output armot
    std::cout << "\n" << "They are "; 
    if (isEnemy()) // output enemy status
    {
        std::cout<<"an enemy.";
    }else{
        std::cout<<"not an enemy.";
    }
    std::cout <<"\n" << "Main Weapon: "<< getMainWeapon(); // output main weapon
    std::cout <<"\n" <<"Offhand Weapon: "<< getSecondaryWeapon(); // output secondary weapon
    std::cout <<"\n" << "Enraged: "; //out put enraged status
    if (getEnrage()){
        std::cout<<"TRUE";
    }else{
        std::cout<<"FALSE";
...
{
    // if barbarian is undead race, they gain 3 vitality points
    if (getRace() == "UNDEAD"){
        setVitality(getVitality()+3);
    }else{
        // if barbarian is any other class, set to vitality to 1
        setVitality(1);
        //switches the enrage status
        if(getEnrage()){
            setEnrage(false);
        }else{
            setEnrage(true);
